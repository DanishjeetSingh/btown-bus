...
export default function Home() {
  ...
  const handleViewportChanged = useCallback(() => setRecenterVisible(true), []);
  const handleRecenterComplete = useCallback(() => setRecenterVisible(false), []);
    ...
    <main className="tracker-shell">
      ...
      <section className={`map-stage ${sheetCollapsed ? 'sheet-collapsed' : 'sheet-open'}`} aria-label="Live Bloomington transit map">
        ...
        <div className={`map-controls ${recenterVisible ? '' : 'compact'}`}>
          ...
          <button className="map-location-button" type="button" onClick={locate} aria-label="Use my location"><LocationIcon /></button>
        </div>
      </section>
      ...
      <section className={`arrival-sheet ${sheetCollapsed ? 'collapsed' : ''}`}>
        ...
        <div className="sheet-body" id="arrival-sheet-body" aria-hidden={sheetCollapsed}>
...
function FitRoutesIcon() {
  return <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M8 4H4v4M16 4h4v4M20 16v4h-4M8 20H4v-4" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" /></svg>;
}

function LocationIcon() {
  return <svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="5.5" fill="none" stroke="currentColor" strokeWidth="2.2" /><path d="M12 2v3M12 19v3M2 12h3M19 12h3" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" /><circle cx="12" cy="12" r="1.6" fill="currentColor" /></svg>;
}
